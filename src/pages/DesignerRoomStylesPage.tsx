...
const DesignerRoomStylesPage: React.FC = () => {
  ...
  const [filters, setFilters] = useState<Filters>({ styles: [], roomTypes: [], designers: [] });
  ...
  const fetchFilters = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/public/design-looks/filters`);
      if (response.ok) {
  ...
  const fetchDesignLooks = async () => {
    try {
      ...
      if (selectedDesigner !== 'all') params.append('designer_id', selectedDesigner);

      const response = await fetch(`${API_BASE_URL}/api/public/design-looks?${params}`);
      if (response.ok) {
        const data = await response.json();
        setDesignLooks(data);
      }
  ...
  const loadLookDataOptimized = useCallback(async (lookId: string) => {
    ...
    try {
      ...
      const productIds = [...new Set(allSpots.map(spot => spot.product_id).filter(Boolean))];
  ...
  const fetchLookImages = async (lookId: string): Promise<DesignLookImage[]> => {
    const response = await fetch(`${API_BASE_URL}/api/public/design-looks/${lookId}/images`);
    if (response.ok) {
  ...
  const fetchAllLookSpots = async (lookId: string): Promise<ImageSpot[]> => {
    ...
    for (const image of images) {
      try {
        const response = await fetch(`${API_BASE_URL}/api/public/design-looks/${lookId}/spots/${image.id}`);
        if (response.ok) {
  ...
  const fetchProductsByIds = async (productIds: string[]): Promise<Product[]> => {
    const params = new URLSearchParams();
    productIds.forEach(id => params.append('product_ids', id));
    
    const response = await fetch(`${API_BASE_URL}/api/public/design-looks/products?${params}`);
    if (response.ok) {
      return response.json();
    }
    return [];
  ...
  const handleImageNavigation = (lookId: string, direction: 'prev' | 'next') => {
    ...
    if (direction === 'prev') {
      newIndex = currentIndex === 0 ? lookData.images.length - 1 : currentIndex - 1;
    } else {
      newIndex = currentIndex === lookData.images.length - 1 ? 0 : currentIndex + 1;
    }
  ...
  const getPopupPositionWithArrow = (spot: ImageSpot, lookId: string) => {
    ...
    const spotX = rect.left - containerRect.left + (rect.width * spot.x_percentage / 100);
    const spotY = rect.top - containerRect.top + (rect.height * spot.y_percentage / 100);
    ...
    if (spaceAbove >= popupHeight + 20) {
      // Position above
      position = {
        left: `${Math.max(10, Math.min(spotX - popupWidth/2, containerRect.width - popupWidth - 10))}px`,
      ...
      arrowStyle = {
        position: 'absolute',
        bottom: '-8px',
        left: `${Math.max(20, Math.min(spotX - (spotX - popupWidth/2 < 10 ? 10 : Math.min(spotX - popupWidth/2, containerRect.width - popupWidth - 10)), popupWidth - 20))}px`,
      ...
      position = {
        left: `${spotX + 15}px`,
        top: `${Math.max(10, Math.min(spotY - popupHeight/2, containerRect.height - popupHeight - 10))}px`,
      };
      arrowClass = 'border-r-white border-t-transparent border-b-transparent';
      arrowStyle = {
        position: 'absolute',
        left: '-8px',
        top: `${Math.max(20, Math.min(spotY - (spotY - popupHeight/2 < 10 ? 10 : Math.min(spotY - popupHeight/2, containerRect.height - popupHeight - 10)), popupHeight - 20))}px`,
  ...
  const mapApiProductToCartProduct = (apiProduct: Product, designer: string): any => {
    return {
      id: apiProduct.id,
      ...
      currency: 'AED',
      images: [
        apiProduct.primary_image || apiProduct.image_url || '',
        ...(apiProduct.additional_images || [])
      ].filter(Boolean),
      colors: apiProduct.color ? [{ name: apiProduct.color, hex: '#000000' }] : [{ name: 'Default', hex: '#000000' }],
  ...
  const handleAddCompleteRoom = (look: DesignLook) => {
    ...
    uniqueProducts.forEach(({ product, count }) => {
      const mappedProduct = mapApiProductToCartProduct(product, look.designer.display_name);
  ...
  const calculateTotalPrice = (lookId: string) => {
    ...
    return uniqueProducts.reduce((sum, { product, count }) => sum + (product.price * count), 0);
  };
  ...
  const getCurrentImageSpots = useMemo(() => {
    return (lookId: string) => {
      ...
      return lookData.allSpots.filter(spot => spot.design_look_image_id === currentImage.id);
    };
  }, [looksData]);
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (lookId && !looksData[lookId]?.isLoaded && !looksData[lookId]?.isLoading) {
  ...
  return (
    <div className="py-12 bg-gray-50">
      <div className="container-custom">
        ...
        >
          <h1 className="font-serif text-4xl md:text-5xl mb-4">Designer Room Styles</h1>
          <p className="text-gray-600 max-w-3xl mx-auto text-lg">
            Complete room packages curated by our expert interior designers. Each collection is thoughtfully 
            designed to create harmonious spaces that reflect your personal style.
          </p>
        </motion.div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-medium text-gray-900">Filter Collections</span>
          </div>
            ...
            <div>
              ...
              >
                ...
                {filters.designers.map(designer => (
                  <option key={designer.id} value={designer.id}>{designer.display_name}</option>
                ))}
              </select>
            </div>
          </div>
              ...
              return (
                ...
                >
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="relative w-full h-full">
                          {lookData?.isLoading ? (
                            ...
                            <img 
                              ...
                              src={currentImage?.image_url || look.mood_board_url} 
                          ...
                          {lookData?.images && lookData.images.length > 1 && (
                            <>
                              <button
                                onClick={() => handleImageNavigation(look.id, 'prev')}
                              ...
                              <button
                                onClick={() => handleImageNavigation(look.id, 'next')}
                          ...
                          {lookData?.isLoaded && spots.map((spot, spotIndex) => {
                            const hasProduct = !!spot.product_id;
                            const isHovered = hoveredSpotId === spot.id;
                            const { position: popupPosition, arrowClass, arrowStyle } = getPopupPositionWithArrow(spot, look.id);
                                    ...
                                    <div className="bg-white rounded-lg shadow-xl p-4 w-70 border border-gray-100">
                                      <div className="flex items-start space-x-3 mb-3">
                                        <div className="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                                          <img
                                            src={spot.product.primary_image || spot.product.image_url}
                                            alt={spot.product.name}
                                            className="w-full h-full object-cover"
                                          />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                          <h5 className="font-medium text-gray-900 text-sm mb-1 line-clamp-2">{spot.product.name}</h5>
                                          <p className="text-xs text-gray-500 mb-2">{spot.product.brand || spot.product.vendor?.business_name}</p>
                                          <p className="text-lg font-bold text-gray-900"><img src="/ed.png" className='w-[20px] inline-block' alt="" /> {spot.product.price.toLocaleString()}</p>
                                        </div>
                                      </div>
                      ...
                      {lookData?.images && lookData.images.length > 1 && (
                        <div className="flex space-x-2 overflow-x-auto">
                          {lookData.images.map((image, imageIndex) => (
                            <button
                              key={image.id}
                              onClick={() => handleThumbnailClick(look.id, imageIndex)}
                              className={`flex-shrink-0 w-16 h-16 rounded-md overflow-hidden border-2 transition-all ${
                                lookData.currentImageIndex === imageIndex ? 'border-black' : 'border-gray-200 hover:border-gray-400'
                              }`}
                            >
                              <img
                                src={image.image_url}
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <p className="text-sm text-gray-700 italic">"{look.description || 'A carefully curated design look.'}"</p>
                        <p className="text-xs text-gray-600 mt-2">— {look.designer.display_name}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div>
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            <h3 className="font-serif text-2xl text-gray-900 mb-2">{look.title}</h3>
                            <div className="flex items-center gap-3 mb-3">
                              <div className="flex items-center text-sm text-gray-600">
                                <User className="w-4 h-4 mr-1" />
                                <span>{look.designer.display_name}</span>
                              </div>
                              {look.style && <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full">{look.style}</span>}
                              {look.room_type && <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full">{look.room_type}</span>}
                            </div>
                            <p className="text-gray-600">{look.description || 'A beautifully curated design look.'}</p>
                          </div>
                        </div>
                        ...
                        <div className="mb-6">
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            ...
                            {lookData?.images && lookData.images.length > 1 && (
                              <div className="flex items-center text-sm text-gray-600">
                                <CheckCircle className="w-4 h-4 text-stone-500 mr-3 flex-shrink-0" />
                                {lookData.images.length} different styling variations
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="bg-stone-50 p-6 rounded-lg mb-6">
                          <div className="flex items-center justify-between">
                            <span className="font-medium text-gray-900">Complete Package Price</span>
                            <div className="text-right">
                              {lookData?.isLoading ? (
                                ...
                                <>
                                  <p className="font-bold text-2xl text-gray-900"><img src="/ed.png" className='w-[20px] inline-block' alt="" /> {totalPrice.toLocaleString()}</p>
                        ...
                        <div className="flex gap-3 mb-4">
                          <button 
                            ...
                            disabled={addedLooks.includes(look.id) || !lookData?.isLoaded || uniqueProducts.length === 0}
                            className={`flex-1 py-3 px-6 font-medium transition-colors text-sm uppercase tracking-wide flex items-center justify-center ${
                              addedLooks.includes(look.id)
                                ...
                                : !lookData?.isLoaded || uniqueProducts.length === 0
                          ...
                          <button 
                            ...
                            className={`px-6 py-3 border border-gray-300 hover:bg-gray-50 transition-colors text-sm font-medium ${
                              lookData?.isLoading ? 'opacity-50 cursor-not-allowed' : ''
                            }`}
                          >
                            {lookData?.isLoading ? (
                              <div className="flex items-center space-x-2">
                              ...
                              expandedLook === look.id ? 'Hide Items' : 'View Individual Items'
                            )}
                          </button>
                  ...
                  {expandedLook === look.id && (
                    ...
                    >
                      <h4 className="font-medium text-gray-900 mb-6">Individual Items in This Collection</h4>
                      {!lookData?.isLoaded ? (
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-gray-600">No products available for this look.</p>
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                              ...
                              <div className="p-4">
                                <div className="flex items-center mb-2">
                                  <div className="flex items-center">
                                    ...
                                    <span className="font-medium ml-1 text-sm">4.5</span>
                                  </div>
                                ...
                                <p className="text-sm text-gray-500 mb-1">{product.brand || product.vendor?.business_name}</p>
                                <h5 className="font-medium text-base mb-3">{product.name}</h5>
                                  ...
                                  <div>
                                    ...
                                    {count > 1 && (
                                      <p className="text-sm font-medium text-gray-700">
                                        Total: <img src="/ed.png" className='w-[20px] inline-block' alt="" /> {(product.price * count).toLocaleString()}
                                      </p>
                                    )}
                                ...
                                <AddToCartButton 
                                  product={mapApiProductToCartProduct(product, look.designer.display_name)}
                                  selectedColor={product.color ? { name: product.color, hex: '#000000' } : { name: 'Default', hex: '#000000' }}
                                  quantity={count}
                                  variant="secondary"
                                  size="sm"
        ...
        {!loading && designLooks.length === 0 && (
          <div className="text-center py-12">
            <Package className="w-16 h-16 text-gray-400 mx-auto mb-4" />
            <h3 className="text-xl font-medium text-gray-900 mb-2">No design looks found</h3>
