...
const CategoryPage: React.FC = () => {
  ...
  const transformProduct = (apiProduct: any): Product => {
    // Handle colors - use both 'colors' array and 'color' text fields from your database
    let productColors = [];
    ...
    const primaryImageUrl = apiProduct.primary_image || 
                           apiProduct.front_image_url || 
                           apiProduct.image_url ||
                           (apiProduct.primary_image?.image_url);
    ...
    const originalPrice = apiProduct.retail_price || apiProduct.price_range?.compare_at_min || undefined;
    ...
    return {
      ...
      reviews: 12, // Default since not in API
      stock: apiProduct.stock_quantity || apiProduct.total_inventory || 0,
      description: apiProduct.description || "",
      inStock: (apiProduct.stock_quantity || apiProduct.total_inventory || 0) > 0,
      featured: false,
      isNew: apiProduct.new_in || apiProduct.is_on_sale || false,
      stockQuantity: apiProduct.stock_quantity || apiProduct.total_inventory || 0,
...
const fetchProductsFromNewAPI = async (roomType: string, subcategory?: string) => {
  ...
  // ✅ Fixed: Added full URL with correct base URL
  const url = subcategory 
    ? `https://marahib-dashboard.vercel.app/api/category/${roomType}/${subcategory}`
    : `https://marahib-dashboard.vercel.app/api/category/${roomType}`;
  ...
  try {
    ...
    if (json.result === "success" && json.data) {
      ...
      console.log(`✅ SUCCESS: Set ${transformedProducts.length} products from new API`);
      console.log("Sample product names:", transformedProducts.slice(0, 3).map(p => p.name));
    } else {
    ...
    // Don't fallback to old API - show empty results instead
    setAllProducts([]);
    setFilteredProducts([]);
  } finally {
    setLoading(false);
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch(`https://marahib-dashboard.vercel.app/api/categories`);
  ...
  const fetchSubcategoriesForCategory = async (categoryId: string) => {
    try {
      const response = await fetch(
        `https://marahib-dashboard.vercel.app/api/subcategories/${categoryId}`
      );
      if (!response.ok) throw new Error(`Response status: ${response.status}`);
      const json = await response.json();
  ...
  const getAllProducts = async () => {
    ...
    const url = `https://marahib-dashboard.vercel.app/api/products`;
  ...
  const getProductsBySubcategory = async (subcategoryParam: string) => {
    ...
    const url = `https://marahib-dashboard.vercel.app/api/products?s_c=${subcategoryParam}`;
    console.log("API URL:", url);
  ...
  const getProductsBySubSubcategory = async (subSubcategoryParam: string) => {
    ...
    const url = `https://marahib-dashboard.vercel.app/api/products?s_s_c=${subSubcategoryParam}`;
    console.log("API URL:", url);
  ...
  const getCategoryDisplayName = () => {
    ...
    return category ? category.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : "Products";
  };

  const getSubcategoryDisplayName = () => {
    if (subcategory && category) {  
      const navbarRoomTypes = ['living-room', 'bedroom', 'dining-room', 'office', 'kitchen', 'entryway', 'outdoor'];
...
useEffect(() => {
  ...
  // List of navbar room types that should use the new API
  const navbarRoomTypes = ['living-room', 'bedroom', 'dining-room', 'office', 'kitchen', 'entryway', 'outdoor'];
  console.log("Available navbar room types:", navbarRoomTypes);
  
  // Check if category matches any navbar room type
  const isNavbarRoomType = category && navbarRoomTypes.includes(category);
  console.log("Is navbar room type?", isNavbarRoomType);
  console.log("Category matches check:", navbarRoomTypes.map(type => ({ type, matches: category === type })));
  
  if (category && navbarRoomTypes.includes(category)) {
    console.log(`✅ CONDITION MET: Using NEW API for navbar room type: ${category}`);
    console.log(`Subcategory: ${subcategory || 'none'}`);
    console.log("About to call fetchProductsFromNewAPI...");
    
    // Check if function exists
    console.log("fetchProductsFromNewAPI function exists:", typeof fetchProductsFromNewAPI);
    
    try {
      // Use new API for navbar-generated URLs
    ...
    // Use existing logic for other URLs  
    if (category === "all") {
      console.log("→ Fetching all products (category = 'all')");
      getAllProducts();
    ...
    <div className="py-12 bg-primary-50">
      <div className="container-custom">
        ...
        {currentSubcategories.length > 0 &&
          !subcategory &&
          category !== "all" && 
          !['living-room', 'bedroom', 'dining-room', 'office', 'kitchen', 'entryway', 'outdoor'].includes(category || '') && (
            ...
            <div className="flex flex-wrap items-center gap-6">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  <option value="0-1000">Under <img src="/ed.png" className='w-[20px] inline-block' alt="" /> 1,000</option>
                  <option value="1000-3000"><img src="/ed.png" className='w-[20px] inline-block' alt="" /> 1,000 - 3,000</option>
                  <option value="3000-5000"><img src="/ed.png" className='w-[20px] inline-block' alt="" /> 3,000 - 5,000</option>
                  <option value="5000-">Over <img src="/ed.png" className='w-[20px] inline-block' alt="" /> 5,000</option>
                </select>
              </div>
