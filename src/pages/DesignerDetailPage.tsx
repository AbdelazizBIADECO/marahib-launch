...
const DesignerDetailPage: React.FC = () => {
  ...
  const fetchDesignerLooks = async (designerId: string) => {
    try {
      ...
      const response = await fetch(
        `https://marahib-dashboard.vercel.app/api/website/designers/looks/${designerId}`,
        {
          method: "GET",
          headers: {
  ...
  const fetchLookImages = async (lookId: string) => {
    try {
      const response = await fetch(
        `https://marahib-dashboard.vercel.app/api/public/design-looks/${lookId}/images`
      );
      if (response.ok) {
  ...
  const fetchImageSpots = async (
    ...
    try {
      const response = await fetch(
        `https://marahib-dashboard.vercel.app/api/public/design-looks/${lookId}/spots/${imageId}`
      );
      if (response.ok) {
  ...
  const loadLookData = async (lookId: string) => {
    ...
    try {
      ...
      if (productIds.length > 0) {
        const response = await fetch(
          `https://marahib-dashboard.vercel.app/api/public/design-looks/products?${new URLSearchParams(
  ...
  const mapApiProductToCartProduct = (
    ...
    return {
      ...
      currency: "AED",
      images: [apiProduct.primary_image || apiProduct.image_url || ""].filter(
        Boolean
      ),
  ...
  useEffect(() => {
    const fetchDesigner = async () => {
      try {
        setLoading(true);
        const response = await fetch(
          `https://marahib-dashboard.vercel.app/api/website/designers/${id}`
        );
